...
async def get_todo(todo_id: int = Path(ge=1), db: AsyncSession = Depends(get_db),current_user:User = Depends(auth_service.get_current_user)):
...
async def create_todo(body: TodoSchema, db: AsyncSession = Depends(get_db),current_user:User = Depends(auth_service.get_current_user)):
    todo = await repositories_todos.create_todo(body, db)
    return todo


@router.put("/{todo_id}")
async def update_todo(body: TodoUpdateSchema, todo_id: int = Path(ge=1), db: AsyncSession = Depends(get_db),current_user:User = Depends(auth_service.get_current_user)):
...
async def delete_todo(todo_id: int = Path(ge=1), db: AsyncSession = Depends(get_db),current_user:User = Depends(auth_service.get_current_user)):
